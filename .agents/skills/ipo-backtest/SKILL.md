...
> **重要规则**：总分 = 基础分 = 100分制。不添加任何不在7维度框架内的"bonus"分数。Whitelist 门槛：基础分 ≥ 80。
...
> 注意：`base_score` 为 7 维度得分之和，满分 100。无 `bonus_score` 字段。
