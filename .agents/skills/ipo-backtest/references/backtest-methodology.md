...
| stock_code | 港股代码 |
| company_name | 公司名称 |
| base_score | 基础分（满分100），7维度之和 |
| classification | 白名单（≥80）/ 灰名单（60-79）/ 黑名单（<60） |
...
> **总分 = 基础分 = 100分制**。评分框架为7维度，无额外bonus项。白名单门槛为基础分 ≥ 80。
...
| 评分-收益相关系数 | Pearson(base_score, first_day_return) | 正值且显著则有效 |
| 高分组收益 | 基础分前50%的平均首日涨幅 | 应显著高于低分组 |
| 低分组收益 | 基础分后50%的平均首日涨幅 | — |
